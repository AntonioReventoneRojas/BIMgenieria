...
res = forms.alert("Caida de tensión por el método de impedancia."
                  "\n\n"
...
if res == "Continuar":
    ...
    datafile = script.get_document_data_file("Caida_Tension_Impedancia", "json")
    ...
    print("""2 hilos
            %e = (200 * In * L(R cosθ + X senθ)) / (CF * Efn)""")

    output.print_md("## Circuito Monofásico 3 hilos (Bifásico)")
    print("""%e = (200 * In * L(R cosθ + X senθ)) / (CF * Eff)""")

    output.print_md("## Circuito Trifásico")
    print("""%e = (173 * In * L(R cosθ + X senθ)) / (CF * Eff)""")

    output.print_md("###Donde:")
    print(""" %e: Caída de tensión en porcentaje.
    L: Longitud del conductor (Kilómetros).
    In: Corriente nominal (Amp.).
    CF: Número de conductores por fase.
    R: Resistencia (Ohms/km). (Valor obtenido de la tabla 9 de la NOM o en su caso de la IEEE-141)
    X: Reactancia (Ohms/km). (Valor obtenido de la tabla 9 de la NOM o en su caso de la IEEE-141)
    Efn: Tensión del sistema fase-neutro (Volts).
    Eff: Tensión del sistema entre fases (Volts).
    θ: Ángulo de desfasamiento entre la tensión y la corriente.
